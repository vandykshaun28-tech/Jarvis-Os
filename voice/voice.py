...
class JarvisVoice:
    ...
    def __init__(self, on_speaking_start=None, on_speaking_end=None):
        # Optional callbacks fired (from the voice worker thread) right
        # before speech starts and right after it finishes. The HUD uses
        # these to mute the microphone while JARVIS is talking, so he
        # doesn't hear himself and reply to his own voice.
    ...
    def _start_engine(self):
        ...
        self._proc = subprocess.Popen(
            ...
            stdout=subprocess.PIPE,   # PowerShell reports '__DONE__' after each utterance
    ...
    def _write_and_wait(self, text: str):
        """Send one utterance and block (in the voice worker thread only)
        until PowerShell confirms it finished speaking."""
        encoded = base64.b64encode(text.encode("utf-8")).decode("ascii")
        self._proc.stdin.write(encoded + "\n")
        self._proc.stdin.flush()
        # Wait for the '__DONE__' acknowledgement. If the process dies,
        # readline() returns '' and we stop waiting.
        while True:
